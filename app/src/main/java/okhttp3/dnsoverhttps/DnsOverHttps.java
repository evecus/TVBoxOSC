...
public class DnsOverHttps implements Dns {
    ...
    private List<InetAddress> readResponse(String hostname, Response response) throws Exception {
        if (response.cacheResponse() == null && response.protocol() != Protocol.HTTP_2) {
            android.util.Log.w("DnsOverHttps", "Incorrect protocol: " + response.protocol());
        }
    ...
    static boolean isPrivateHost(String host) {
        // 检测私有 IP 地址或本地主机名（替代已移除的 okhttp3.internal.publicsuffix.PublicSuffixDatabase）
        if (host == null || host.isEmpty()) return true;
        if (host.equals("localhost") || host.endsWith(".local") || host.endsWith(".internal")) return true;
        // 私有 IPv4 段: 10.x, 172.16-31.x, 192.168.x
        if (host.matches("^10\\..*") || host.matches("^172\\.(1[6-9]|2[0-9]|3[01])\\..*")
                || host.matches("^192\\.168\\..*") || host.equals("127.0.0.1")) return true;
        return false;
    }
